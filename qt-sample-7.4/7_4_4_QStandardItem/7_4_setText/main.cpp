...
int main(int argc, char *argv[])
{
   ...
   parent->setTextAlignment(Qt::AlignRight);
   QIcon ico = window.style()->standardIcon(QStyle::SP_MessageBoxCritical);
   parent->setIcon(ico);
   parent->setToolTip("Текст подсказки");
   parent->setFont(QFont("Verdana", 16));
   parent->setBackground(QBrush(QColor("black")));
   parent->setForeground(QBrush(QColor("white")));

   QStandardItem *item2 = new QStandardItem("Пункт 2");
   item2->setCheckable(true);
   item2->setCheckState(Qt::Unchecked);
   model->appendRow(item2);

   QStandardItem *item3 = new QStandardItem("Пункт 3");
